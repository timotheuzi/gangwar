...
# Determine the project root directory (where src/ is located)
if [ -f "src/app.py" ]; then
    PROJECT_ROOT="."
else
    PROJECT_ROOT="../"
    if [ ! -f "${PROJECT_ROOT}/src/app.py" ]; then
        echo "Error: src/app.py not found. Please run this script from the project root or scripts/ directory within a gangwar project."
        exit 1
    fi
fi
...
if [ "$PROJECT_ROOT" = "." ]; then
    ...
    # We're in scripts directory, need absolute path
    PYTHON_FILE="${PROJECT_ROOT}/src/app.py"
    cd "$PROJECT_ROOT"
fi

# Start the Python application
exec "$PYTHON_EXE" "$PYTHON_FILE"
